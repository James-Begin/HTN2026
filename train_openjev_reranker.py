"""Fine-Tuning OpenJev-4B for 5-Class Social Claim Reranking on H100.

Implements:
1. Qwen3.5-4B Sequence Classification with 5-class head and LoRA adaptation (r=16, alpha=32).
2. Asymmetric Margin BCE + Weighted Cross Entropy.
3. Multi-stage Hard Negative Mining.
4. Distillation loss against BGE-Large soft distributions.
5. Full evaluation across 25-pair gate, 170-pair Gold Test Suite, and 1,483-pair Held-Out Validation split.
"""
import gc
import json
import math
...
def main():
    ...
    print("\n" + "="*80, flush=True)
    print(">>> FINE-TUNING OPENJEV-4B WITH LORA AND 5-CLASS RERANKING HEAD", flush=True)
    print("="*80, flush=True)
    ...
    best_auc = 0.0
    history = []
    jev_out = OUT / "openjev-finetuned-best"
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, batch in enumerate(loader, 1):
            ...
            if step % 200 == 0 or step == len(loader):
                print(f"[openjev-ft] epoch={epoch} step={step}/{len(loader)} loss={total_loss / step:.4f} lr={scheduler.get_last_lr()[0]:.2e}", flush=True)

        val_auc = evaluate_openjev_val(model, val_loader)
        ep_time = time.time() - t0
        print(f"[openjev-ft epoch {epoch}] train_loss={total_loss/len(loader):.4f} | val_auc={val_auc:.4f} | time={ep_time:.1f}s", flush=True)
        ...
        if val_auc > best_auc:
            ...
            print(f"[openjev-ft] Saved best checkpoint (val_auc={best_auc:.5f})", flush=True)

    # Evaluate best model
    print("\n" + "="*80, flush=True)
    print(">>> EVALUATING FINE-TUNED OPENJEV-4B ON TEST BENCHMARKS", flush=True)
    print("="*80, flush=True)
    ...
    print(f"[openjev-ft 25-pair] ROC_AUC={res_25['metrics']['roc_auc']:.4f} | F1={res_25['metrics']['f1_at_0.5']:.4f}", flush=True)
    print(f"[openjev-ft 25-targets] {json.dumps(res_25['targets'])}", flush=True)
    print(f"[openjev-ft 170-pair] ROC_AUC={res_170['metrics']['roc_auc']:.4f} | F1={res_170['metrics']['f1_at_0.5']:.4f}", flush=True)

    # Latency / throughput test
    t_start = time.time()
    sample_100 = (eval_170_rows * 2)[:100]
    _ = score_openjev_set(eval_model, tok, template, sample_100)
    elapsed = time.time() - t_start
    throughput = len(sample_100) / elapsed
    print(f"[openjev-ft speed] {throughput:.1f} pairs/sec | Latency: {1000/throughput:.2f} ms/pair", flush=True)

    result = {
        "model": "AlexWortega/openjev (Fine-Tuned Qwen3.5-4B LoRA 5-Class)",
        "best_val_auc": best_auc,
        "history": history,
        "eval_25": res_25,
        "eval_170": res_170,
        "speed": {"pairs_per_sec": throughput, "ms_per_pair": 1000/throughput}
    }
    write_json(RESULTS / "openjev_finetuned.json", result)
    print("\nOPENJEV FINE-TUNING COMPLETED SUCCESSFULLY!", flush=True)
