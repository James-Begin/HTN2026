"""4-Bit Quantized (QLoRA) OpenJev-4B Training, Distillation & Latency Benchmark.

Features:
1. 4-bit NormalFloat (NF4) quantization via bitsandbytes (drops base model VRAM from 8.5GB to 2.4GB).
2. QLoRA with rank 32, alpha 64 covering all Qwen3.5 linear projections.
3. Asymmetric Margin BCE + Weighted Cross-Entropy + Dynamic Hard Negative Mining.
4. Latency, memory, and throughput benchmarking across batch sizes [1, 4, 8, 16, 32, 64, 128].
5. Full evaluation across 25-pair Gate, 170-pair Gold Test Suite, and 1,483-pair Val Set.
"""
import gc
import json
import math
...
def mine_hard_negatives_openjev(model, tok, template, rows, limit=512):
...
def benchmark_4bit_latency(model, tok, template):
    print("\n" + "="*70 + "\nBenchmarking 4-Bit Quantized OpenJev-4B Latency & Throughput\n" + "="*70, flush=True)
    pairs = [
    ...
    # Warmup
    for _ in range(10):
        t = template.format(premise=pairs[0][0], hypothesis=pairs[0][1])
        inp = tok(t, return_tensors="pt", max_length=192, truncation=True, padding=True).to("cuda")
        with torch.inference_mode():
            _ = model(**inp).logits
    ...
    for i in range(100):
        p = pairs[i % len(pairs)]
        t = template.format(premise=p[0], hypothesis=p[1])
        inp = tok(t, return_tensors="pt", max_length=192, truncation=True, padding=True).to("cuda")
        torch.cuda.synchronize()
        start_event.record()
        with torch.inference_mode():
    ...
    for bs in [1, 4, 8, 16, 32, 64, 128]:
        batch_pairs = (pairs * (bs // len(pairs) + 1))[:bs]
        texts = [template.format(premise=p[0], hypothesis=p[1]) for p in batch_pairs]
        ...
        tps = (bs * 1000.0) / avg_ms
        batch_res[str(bs)] = {
            "batch_size": bs,
            "avg_batch_latency_ms": round(avg_ms, 2),
            "throughput_pairs_per_sec": round(tps, 1)
        }
        print(f"  Batch Size {bs:3d}: {tps:7.1f} pairs/sec (Latency: {avg_ms:6.2f} ms)", flush=True)

    peak_vram_mb = torch.cuda.max_memory_allocated() / (1024**2)
    print(f"  Peak VRAM: {peak_vram_mb:.1f} MB ({peak_vram_mb/1024:.2f} GB)", flush=True)
    return {"single_pair": single_res, "batch_scaling": batch_res, "peak_vram_mb": peak_vram_mb}
...
def main():
    ...
    print("\n" + "="*80, flush=True)
    print(">>> 4-BIT QUANTIZED (NF4) OPENJEV-4B QLORA FINE-TUNING", flush=True)
    print("="*80, flush=True)
    ...
    base_vram_mb = torch.cuda.memory_allocated() / (1024**2)
    print(f"4-Bit Base Model Loaded in {load_time:.2f}s | VRAM: {base_vram_mb:.1f} MB ({base_vram_mb/1024:.2f} GB)", flush=True)

    # Explicitly attach 5-class classification head
    ...
    batch_size = 16
    grad_accum = 2
    est_steps = (epochs * (len(train_rows) + 512) // (batch_size * grad_accum))
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch > 1:
            train_iter.extend(mine_hard_negatives_openjev(model, tok, template, train_rows, limit=512))
        ...
        for step, batch in enumerate(loader, 1):
            ...
            loss = (ce(logits.float(), labels) + 1.2 * bce(same.clamp(1e-6, 1 - 1e-6), is_same)) / grad_accum
            loss.backward()
            ...
            if step % 100 == 0 or step == len(loader):
                print(f"[openjev-4bit] epoch={epoch} step={step}/{len(loader)} loss={total_loss / step:.4f} lr={scheduler.get_last_lr()[0]:.2e}", flush=True)

        val_auc = evaluate_openjev_val(model, val_loader)
        ep_time = time.time() - t0
        print(f"[openjev-4bit epoch {epoch}] train_loss={total_loss/len(loader):.4f} | val_auc={val_auc:.4f} | time={ep_time:.1f}s", flush=True)
        history.append({"epoch": epoch, "train_loss": total_loss/len(loader), "val_auc": val_auc, "seconds": ep_time})

        if val_auc > best_auc:
            best_auc = val_auc
            if jev_out.exists():
                shutil.rmtree(jev_out)
            model.save_pretrained(jev_out)
            tok.save_pretrained(jev_out)
            print(f"[openjev-4bit] Saved best checkpoint (val_auc={best_auc:.5f})", flush=True)

    # Evaluate best model
    print("\n" + "="*80, flush=True)
    print(">>> EVALUATING 4-BIT QUANTIZED OPENJEV-4B ON TEST BENCHMARKS", flush=True)
    print("="*80, flush=True)

    eval_model = model.eval()
    res_25 = score_openjev_set(eval_model, tok, template, eval_25_rows)
    res_170 = score_openjev_set(eval_model, tok, template, eval_170_rows)

    print(f"[openjev-4bit 25-pair] ROC_AUC={res_25['metrics']['roc_auc']:.4f} | F1={res_25['metrics']['f1_at_0.5']:.4f}", flush=True)
    print(f"[openjev-4bit 25-targets] {json.dumps(res_25['targets'])}", flush=True)
    print(f"[openjev-4bit 170-pair] ROC_AUC={res_170['metrics']['roc_auc']:.4f} | F1={res_170['metrics']['f1_at_0.5']:.4f}", flush=True)

    bench_res = benchmark_4bit_latency(eval_model, tok, template)

    result = {
        "model": "AlexWortega/openjev (4-Bit NF4 Quantized QLoRA r=32)",
    ...
    write_json(RESULTS / "openjev_4bit_results.json", result)
    print("\n4-BIT QUANTIZED OPENJEV TRAINING & BENCHMARKING COMPLETE!", flush=True)
