...
3. Set service variables `X_BEARER`, `OPENAI_API_KEY`, and `BASETEN_API_KEY`. Optional model routes: `SEQUITOR_BASETEN_EMBED_URL`, `SEQUITOR_BASETEN_EMBED_MODEL`, `SEQUITOR_BASETEN_CHAIN_URL`, and the existing cross-encoder route variables. Keep all keys in Railway variables; `.env` stays local and ignored.
...
The default server-wide cap is 600 X post reads plus 20 counts calls, roughly $3.20. It can be changed with `SEQUITOR_X_POST_CAP` and `SEQUITOR_X_COUNTS_CAP`, but the cap applies to the persistent Railway ledger, not the local testing ledger. X billing is account-wide, so include the local $3.44 already spent when deciding the remote cap. Use one Railway replica with the mounted volume for this hackathon demo.
